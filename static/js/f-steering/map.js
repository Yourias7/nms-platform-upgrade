...
export function buildMarkerTooltipContent(row, serial) {
  ...
  const geolock = getFieldCaseInsensitive(row, ['geolock', 'GEOLOCK']);
  const nodeId = getFieldCaseInsensitive(row, ['node_id', 'NODE_ID']);
  const sectorId = getFieldCaseInsensitive(row, ['sector_id', 'SECTOR_ID']);
  ...
  return (
    ...
    `<br>Geolock: ${geolock ?? 'N/A'}` +
    `<br>Node ID: ${nodeId ?? 'N/A'}` +
    `<br>Sector ID: ${sectorId ?? 'N/A'}` +
    `<br>TEMP: ${tempVal}°C` +
