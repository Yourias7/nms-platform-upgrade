...
/**
 * Determine F-Steering performance LED color.
 * Performance alarm is only GPS/geolock or TEMP above threshold.
 * @param {number|null} temp
 * @param {number|null} lat
 * @param {number|null} lon
 * @param {string|null} geolock
 * @returns {string} CSS class name ('led-green' or 'led-red')
 */
function getLEDClass(temp, lat, lon, geolock) {
  if (isInAlarm('temp', temp) || isInAlarm('geolock', geolock) || isInAlarm('lat', lat) || isInAlarm('lon', lon)) {
    return 'led-red';
  }
  return 'led-green';
}
...
export async function renderSerials(data, onSelectSerial, loadMultipleDetails = null) {
  ...
  for (const s of data) {
    ...
    try {
      ...
      const geolock = latest ? getFieldCaseInsensitive(latest, ['geolock', 'GEOLOCK']) : null;
      const datetime = latest ? getFieldCaseInsensitive(latest, ['datetime', 'DATETIME']) : null;

      const ledClass = getLEDClass(temp, lat, lon, geolock);
