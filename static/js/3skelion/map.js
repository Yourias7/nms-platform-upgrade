...
export function buildMarkerTooltipContent(row, serial) {
  // =========================================
  // 3skelion tooltip: PowerBI-like fields only
  // =========================================

  // SHIP: έρχεται από backend ως computed field (ή/και ως NAME alias)
  const ship = getFieldCaseInsensitive(row, ['ship', 'SHIP', 'name', 'NAME']) ?? 'ERROR';

  // Coordinates (στο backend έχεις LAT/LON αλλά έχουμε και aliases LATITUDE/LONGITUDE)
  ...
  const snr  = getFieldCaseInsensitive(row, ['best_snr', 'BEST_SNR', 'snr', 'SNR', 'sinr', 'SINR']);

  // Extra info
  const cellid   = getFieldCaseInsensitive(row, ['best_cellid', 'BEST_CELLID', 'cellid', 'CELLID', 'cid', 'CID']);
  const antenna  = getFieldCaseInsensitive(row, ['best_antenna', 'BEST_ANTENNA', 'antenna', 'ANTENNA']);
  ...
  const rsrpVal = (rsrp !== null && rsrp !== undefined) ? safeParseFloat(rsrp).toFixed(1) : 'N/A';
  const rsrqVal = (rsrq !== null && rsrq !== undefined) ? safeParseFloat(rsrq).toFixed(1) : 'N/A';
  const snrVal  = (snr  !== null && snr  !== undefined) ? safeParseFloat(snr).toFixed(2)  : 'N/A';
  const tempVal = (temp !== null && temp !== undefined) ? safeParseFloat(temp).toFixed(1) : 'N/A';
