...
async function countCommunicationAlarms() {
  try {
    let serials;
    try {
      serials = await fetchJSON(CONFIG.API.SERIALS);
    } catch (err) {
      console.warn('[Alarm Badge] Failed to fetch serials list - backend may be unavailable:', err.message);
      return 0;
    }

    if (!Array.isArray(serials) || serials.length === 0) {
      console.debug('[Alarm Badge] No serials found');
      return 0;
    }
