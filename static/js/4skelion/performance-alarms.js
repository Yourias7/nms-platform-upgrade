...
function getVisiblePerformanceAlarms() {
  ...
  if (lassoFilteredAlarmKeys && lassoFilteredAlarmKeys.size > 0) {
    alarms = alarms.filter(a => lassoFilteredAlarmKeys.has(String(a.key)));
  }
...
function toggleTypeFilter(key) {
  ...
  currentPage = 1;
  renderPerformanceAlarmsTable(getVisiblePerformanceAlarms(), null);
...
function renderPerformanceAlarmsTable(alarms, total = null) {
  ...
  const sortedAlarms = sortPerformanceAlarms(filteredAlarms, sortColumn, sortDirection);
  ...
  window.dispatchEvent(new CustomEvent('alarms:table-rendered', { detail: { alarms: filteredAlarms } }));
}
...
async function init() {
  ...
  if (clearBtn) {
    ...
    bindTypeFilterButtons(); // Bind type filter buttons after rendering
  }
